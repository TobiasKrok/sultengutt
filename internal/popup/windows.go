...
import (
	...
	"os"
	"path/filepath"
	"sultengutt/internal/mantras"
)

// GenerateWindowsScript creates a modern PowerShell script for the Windows popup
func GenerateWindowsScript(configDir string) (string, error) {
...
func GenerateWindowsFallbackScript(configDir string) (string, error) {
	if err := os.MkdirAll(configDir, 0755); err != nil {
		return "", fmt.Errorf("failed to create config directory: %w", err)
	}
